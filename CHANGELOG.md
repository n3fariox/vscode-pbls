...
- Integrated pbls (Protobuf Language Server) for LSP features
  ...
  - go-to-definition
  - diagnostics
- Added automatic pbls download if not found on system
- Added "Reload Protobuf Language Server" command for restarting the language server

#### Removed

- Removed built-in DocumentSymbolProvider (now handled by pbls)
- Removed automatic field/enum renumbering on save (`protoc.renumber_on_save`)
- Removed `compile_on_save` feature

#### Unchanged

- clang-format document formatting preserved
